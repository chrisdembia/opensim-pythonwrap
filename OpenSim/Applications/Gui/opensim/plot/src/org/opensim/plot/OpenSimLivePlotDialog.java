 ...
 */
public class OpenSimLivePlotDialog extends JDialog{
  ...
  OpenSimFigure currentFigure = new OpenSimFigure("Figure 1");
  ...
  JPopupMenu xPopup = new JPopupMenu();//SimtkPlotQuantitySelector(jXTextField);
  JPopupMenu yPopup = new JPopupMenu();//SimtkPlotQuantitySelector(jYTextField);
      ...
      public void actionPerformed(ActionEvent e) {
       ...
       // Strip out the leading environment name
        ...
        if (jRectifyCheckBox.isSelected()) {
          PlotRectifyFilter filter = new PlotRectifyFilter();
          filters.add(filter);
        }
        if (!jScaleTextField.getText().equals("1.0")) {
          PlotScaleFilter filter = new PlotScaleFilter(new Double(
              jScaleTextField.getText()).doubleValue());
          filters.add(filter);
        }
        if (!jOffsetTextField.getText().equals("0.0")) {
          PlotOffsetFilter filter = new PlotOffsetFilter(new Double(
        ...
        jLegendText.setText(PlotDataSet.DEFAULT_LEGEND);
    ...
    jClearFigureButton.addActionListener(new ActionListener(){
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        while (enumElems.hasMoreElements()) {
          PlotDataSet dataSet = (PlotDataSet) enumElems.nextElement();
    ...
    jPrintFigureButton.addActionListener(new ActionListener() {
      ...
      if (job.printDialog(aset)) {
        try {
          ...
          JOptionPane.showMessageDialog(OpenSimLivePlotDialog.this,
                                        "Printing failed:\n" + ex.toString(),
                                        "Print Error",
  ...
  public int getNextAvailableIndex(int figureIndex)
  {
    ...
    while(elements.hasMoreElements()){
      PlotDataSet dataSet = (PlotDataSet) elements.nextElement();
  ...
  void jDeletePlotButton_actionPerformed(ActionEvent e) {
    ...
    for(int i=0; i < selectedForDeletion.length; i++){
      PlotDataSet dataSet = (PlotDataSet) _mapNamesToDatasets.get((String) selectedForDeletion[i]);
      dataSet.cleanup();
      int index = dataSet.getDataSetIndex();
      OpenSimFigure ownerFigure = (OpenSimFigure) figures.get(dataSet.getFigureIndex());
...
class SimtkPtPlotDialog_jDeletePlotButton_actionAdapter implements java.awt.event.ActionListener {
  OpenSimLivePlotDialog adaptee;

  SimtkPtPlotDialog_jDeletePlotButton_actionAdapter(OpenSimLivePlotDialog adaptee) {
...
class SimtkPtPlotDialog_SelectXButton_mouseAdapter extends java.awt.event.MouseAdapter {
  ...
  SimtkPtPlotDialog_SelectXButton_mouseAdapter(OpenSimLivePlotDialog adaptee) {
    this.adaptee = adaptee;
  }
...
class SimtkPtPlotDialog_SelectYButton_mouseAdapter extends java.awt.event.MouseAdapter {
  ...
  SimtkPtPlotDialog_SelectYButton_mouseAdapter(OpenSimLivePlotDialog adaptee) {
    this.adaptee = adaptee;
  }
  ...
  /**
     ...
     * @param figure OpenSimFigure
     * @return boolean
     */
  public boolean legendExists(String aLegend, OpenSimFigure figure)
  {
    return (figure.getLegendDataset(aLegend)!= -1);
  }

  public OpenSimFigure getFigure(int onebasedIndex)
  {
    return (OpenSimFigure) figures.get(onebasedIndex-1);
  }
  ...
  void jSaveTemplateMenuItem_actionPerformed(ActionEvent e) {
    try {
      String fileName = "";//JFileChooser.getFile("Select file to save template to", "xml", "*.xml");
      if (fileName == null)
        return;
  ...
  private void writeFigures(PrintWriter fout) throws IOException  {
    ...
    for (int i=0; i < numFigures; i++){
      OpenSimFigure nextFigure = (OpenSimFigure) figures.get(i);
      ...
      while (dataSets.hasMoreElements()){
        PlotDataSet nextPlot = (PlotDataSet) dataSets.nextElement();
  ...
  private void read(URL base, FileInputStream in) throws IOException {
    ...
    if ((new String(peek)).startsWith("<?xm")) {
        // file is an XML file.
        _parser = new PlotTemplateXMLParser(this);
        try {
  ...
  public void setGridSize(int desiredRows, int desiredColumns) {
    if (desiredRows*desiredColumns < numFigures){
      ...
      for (int i =desiredRows*desiredColumns; i < numFigures; i++){
        OpenSimFigure nextPlot = (OpenSimFigure)figures.get(i);
      ...
      for (int i = numFigures; i < desiredRows * desiredColumns; i++) {
        OpenSimFigure newPlot = new OpenSimFigure("Figure:"+(i+1));
  ...
  public void addDataSet(int zeroBasedCurrentFigureId, String xName, String yName,
   ...
   PlotDataSet newDataSet = new PlotDataSet(xName, yName, userLegend, currentFigureIndex, connect, marks);
   // Add filters to dataset
   for(int i=0; i < filters.size(); i++)
     newDataSet.addFilter((PlotDataFilter) filters.get(i));

   currentFigure = (OpenSimFigure) figures.get(currentFigureIndex);
  ...
  {
    currentFigure = (OpenSimFigure) figures.get(zeroBasedIndex);
  ...
  void exportCurrentPlot() {
    try {
      String fileName = ""; /*new JFileChooser().getFile(
      ...
      for(int i=0; i < selectedForExport.length; i++){
        PlotDataSet dataSet = (PlotDataSet) _mapNamesToDatasets.get((String) selectedForExport[i]);
          OpenSimFigure fig =  (OpenSimFigure) figures.get(dataSet.getFigureIndex());
