/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (http://www.swig.org).
 * Version 1.3.31
 *
 * Do not make changes to this file unless you know what you are doing--modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

package org.opensim.modeling;

public class SimmRotationDof extends AbstractDof {
  private long swigCPtr;

  public SimmRotationDof(long cPtr, boolean cMemoryOwn) {
    super(opensimModelJNI.SWIGStorageUpcast(cPtr), cMemoryOwn);
    swigCPtr = cPtr;
  }

  public static long getCPtr(SimmRotationDof obj) {
    return (obj == null) ? 0 : obj.swigCPtr;
  }

  protected void finalize() {
    delete();
  }

  public synchronized void delete() {
    if(swigCPtr != 0 && swigCMemOwn) {
      swigCMemOwn = false;
      opensimModelJNI.delete_SimmRotationDof(swigCPtr);
    }
    swigCPtr = 0;
    super.delete();
  }

  public SimmRotationDof() {
    this(opensimModelJNI.new_SimmRotationDof__SWIG_0(), true);
  }

  public SimmRotationDof(SimmRotationDof aDof) {
    this(opensimModelJNI.new_SimmRotationDof__SWIG_1(SimmRotationDof.getCPtr(aDof), aDof), true);
  }

  public OpenSimObject copy() {
    long cPtr = opensimModelJNI.SimmRotationDof_copy(swigCPtr, this);
    return (cPtr == 0) ? null : new OpenSimObject(cPtr, false);
  }

  public void copyData(SimmRotationDof aDof) {
    opensimModelJNI.SimmRotationDof_copyData(swigCPtr, this, SimmRotationDof.getCPtr(aDof), aDof);
  }

  public void setAxis(SWIGTYPE_p_SimTK__Vec3 aAxis) {
    opensimModelJNI.SimmRotationDof_setAxis(swigCPtr, this, SWIGTYPE_p_SimTK__Vec3.getCPtr(aAxis));
  }

  public void getAxis(SWIGTYPE_p_SimTK__Vec3 rAxis) {
    opensimModelJNI.SimmRotationDof_getAxis__SWIG_0(swigCPtr, this, SWIGTYPE_p_SimTK__Vec3.getCPtr(rAxis));
  }

  public SWIGTYPE_p_SimTK__Vec3 getAxis() {
    return new SWIGTYPE_p_SimTK__Vec3(opensimModelJNI.SimmRotationDof_getAxis__SWIG_1(swigCPtr, this), false);
  }

  public SWIGTYPE_p_double getAxisPtr() {
    long cPtr = opensimModelJNI.SimmRotationDof_getAxisPtr(swigCPtr, this);
    return (cPtr == 0) ? null : new SWIGTYPE_p_double(cPtr, false);
  }

  public double getValue() {
    return opensimModelJNI.SimmRotationDof_getValue(swigCPtr, this);
  }

  public AbstractDof.DofType getMotionType() {
    return AbstractDof.DofType.swigToEnum(opensimModelJNI.SimmRotationDof_getMotionType(swigCPtr, this));
  }

}
