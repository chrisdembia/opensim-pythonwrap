 ...
 * as a blob.
 *
 * Source Java world: http://www.javaworld.com/javaworld/javatips/jw-javatip70.html
 */
public final class JarResources
	...
	try
		...
		if (debugOn)
		    {
		    SimtkApp.displayDebugMessage(dumpZipEntry(ze));
		    }
		...
		if (debugOn)
		    {
		    SimtkApp.displayDebugMessage("ze.getName()="+ze.getName()+
				       ","+"getSize()="+ze.getSize() );
		    }
		...
		if (debugOn)
		    {
		    SimtkApp.displayDebugMessage( ze.getName()+"  rb="+rb+
					",size="+size+
		...
		{
		SimtkApp.displayDebugMessage("done.");
		}
	...
	if (buff==null)
	    {
	    SimtkApp.displayDebugMessage("Could not find "+args[1]+".");
	    }
	else
	    {
	    SimtkApp.displayDebugMessage("Found "+args[1]+
			       " (length="+buff.length+").");
	    }
	}
