...
#include <RD/Simulation/Model/Actuator.h>
#include <RD/Simulation/Model/ActuatorSet.h>
#include <RD/Simulation/Model/ContactForce.h>
#include <RD/Simulation/Model/ContactForceSet.h>
#include <RD/Simulation/TestSuite/ModelTestSuite.h>
#include <RD/Simulation/Control/ControlLinear.h>
#include <RD/Simulation/Manager/Manager.h>
#include <RD/Models/Block/rdBlock.h>
#include <SU/Analyses/Analyses.h>
...
int main(int argc,char **argv)
{
	...
	// RUN TESTS
	//ModelTestSuite suite;
	//suite.Test(model);
	// TestContacts();

	//__________________________________
	// ----- SIMULATION BELOW HERE -----
	// Jim, the following is not meant to be a part of ModelTestSuite.
	...
	kin->setOn(true);
	// Body Kinematics
	BodyKinematics *bodKin = new BodyKinematics(model);
	...
	//DecompTaylor *taylor = new DecompTaylor(model);
	...
	tn[3] = 7.0;	xn[3] = 0.0;
	tn[4] = tf;		xn[4] = 0.0;
	ControlLinear *m1 = new ControlLinear(nn,tn,xn);
	// muscle 2
	tn[0] = 0.0;	xn[0] = 0.0;
	tn[1] = 6.5;	xn[1] = 0.0;
	tn[2] = 6.51;	xn[2] = 50.0;
	tn[3] = 6.0;	xn[3] = 50.0;
	tn[4] = tf;		xn[4] = 50.0;
	ControlLinear *m2 = new ControlLinear(nn,tn,xn);

	// SET NEW CONTROLS
	Controls *controls = manager->getControls();
	...
	// STORAGE SETTINGS
	Storage *yStore = integ->getStateStorage();
	Storage *ypStore = integ->getPseudoStateStorage();
	Storage *xStore = integ->getControlStorage();
...
void TestContacts()
{
	...
	ContactForceSet *contactSet = new ContactForceSet();
	contactSet->print("default.ctx");

	// CONSTRUCT CONTACTS FROM FILE
	ContactForceSet contactSet1("set1.ctx");
