/*******************************************************************************
   ...
   Routines:
      ...
      muscle_deriv_func8  : 2 states (act, fiber_len), parameterized constituitive equations
      muscle_deriv_func9  : same as func8 except it is assumed that m-t length and velocity are set outside of routine
		                 ...
		                 double dstate[], double* muscle_force)
{
   ...
   *muscle_force = ms->force;
   ...
   return (4); /* return the number of states in the model */
}
		                 ...
		                 double dstate[], double* muscle_force)
{
	...
	*muscle_force = ms->force;
	...
	return (0); /* return the number of states in the model */
}


/* 2 state model that uses paramaterized functions for the active
   and passive force-length relations, the tendon force-strain relation
   and the force-velocity relationships
 */
		       ...
		       double dstate[], double* muscle_force)
{
   ...
   int activation = 0, fiber_length = 1;
   double act, exc, tendon_force, active_force;
   double passive_force, velocity_dependent_force;
   double normstate[2], normdstate[2], norm_tendon_length, ca;
   double norm_muscle_tendon_length, pennation_angle;
   double h, w, new_fiber_length, new_pennation_angle, new_ca;
   double c_act_tau, c_deact_tau, rate;
   double Vmax, Vmax_zero_act, power;

	/* The current normalized muscle states */
	normstate[activation] = state[activation];
	normstate[fiber_length] = state[fiber_length] / (*(ms->optimal_fiber_length));

	// The dynamic parameters used
	c_act_tau = get_muscle_param_value(ms,"activation_time_constant");
	c_deact_tau = get_muscle_param_value(ms,"deactivation_time_constant");
	// Maximum contraction velocity is an activation scaled value
	Vmax = get_muscle_param_value(ms,"Vmax");
	Vmax_zero_act = get_muscle_param_value(ms,"Vmax0");
	if (normstate[activation]<1.0)
		Vmax = Vmax_zero_act + normstate[activation]*(Vmax-Vmax_zero_act);
	Vmax = Vmax*(*(ms->optimal_fiber_length));

   /* Compute normalized muscle state derivatives */
	exc = ms->excitation_level;
	act = normstate[activation];
	if (exc >= normstate[activation])
		rate = 1./c_act_tau;
   else
		rate = 1./c_deact_tau;
   normdstate[activation] = rate*(ms->excitation_level - normstate[activation]);


	pennation_angle = calc_pennation(normstate[fiber_length],1.0,(*(ms->pennation_angle)));
	ca = cos(DEG_TO_RAD*pennation_angle);
	norm_muscle_tendon_length = calc_muscle_tendon_length(ms) / (*(ms->optimal_fiber_length));
//	norm_muscle_tendon_length = ms->muscle_tendon_length / (*(ms->optimal_fiber_length));
	norm_tendon_length = norm_muscle_tendon_length - normstate[fiber_length]*ca;
	tendon_force = calc_tendon_force_dt(ms,norm_tendon_length);
	passive_force = calc_passive_fiber_force_dt(ms,normstate[fiber_length]);
	active_force = calc_active_force_dt(ms,normstate[fiber_length]);
...
//			normdstate[fiber_length] = ms->muscle_tendon_vel / (Vmax*new_ca) ;
		...
		velocity_dependent_force = 0.;
	}
	else {
		velocity_dependent_force = tendon_force/ca - passive_force;
		normdstate[fiber_length] = calc_norm_fiber_velocity_dt(ms,normstate[activation],active_force,velocity_dependent_force);
	}
   ...
   /* Note: Do not need to Un-Normalize activation dynamics equation since activation, deactivation parameters
    ...
    dstate[activation] = normdstate[activation];
    dstate[fiber_length] = Vmax*normdstate[fiber_length];

   /* Store the muscle activation in the muscle structure */  
//	ms->activation = state[activation];
	ms->dynamic_activation = state[activation];
	ms->fiber_length = state[fiber_length];
	ms->fiber_velocity = dstate[fiber_length];
...
int muscle_deriv_func9(double time, dpMuscleStruct* ms, double state[],
		       double dstate[], double* muscle_force)
{
/* func9 is same as number 8 but it is assumed that m-t length and 
   velocity are set prior
 */
