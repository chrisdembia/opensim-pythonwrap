/*******************************************************************************
   ...
   Routines:
      ...
      convert_point2         : converts a point from one frame to another
...
/* convert a point from frame1 to frame2. */
...
void apply_external_forces(double time, MotionData* data)
{
   ...
   for (i = 0; i < sdm->nq; i++)
   {
      if (data->q_torques[i])
      {
         sdhinget(sdm->q[i].joint, sdm->q[i].axis, interpolate_spline(time, data->q_torques[i], zeroth, 1.0, 0.0));
      }
   }
}
 ...
 * the user-specified prescribed motion. It does not include torques from muscles,
 * restraint torques, spring forces, body forces and torques, and any other forces
 * applied in sduforce(). It also does not include constraint torques applied by
 * sduconsfrc(). If you prescribe the motion of the system while applying muscle
 * forces or joint torques that exactly produce that motion, this function will
 * calculate joint torques of zero.
...
void calc_joint_torques(double time, double state[], double dy[])
{
   ...
   /* Form a new multiplier array with non-zero multipliers only for
    * the constrained Qs. This is done so that sdfulltrq() will return
    * zero torques for the constrained Qs, and non-zero torques for the
    * unconstrained ones (and the fixed ones, but those are not output).
    */
   for (i = 0; i < sdm->nq; i++)
   {
