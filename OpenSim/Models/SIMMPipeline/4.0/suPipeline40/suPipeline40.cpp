 ...
 * Constructor.
 *
 * Although this class is derived from ActuatedModel_SDFast, this class
 ...
 * will be added to the actutor set in ActuatedModel_SDFast.
 *
...
suPipeline40::suPipeline40(const string &workpath, const string &paramfile, 
   ActuatorSet *aActuators,ContactForceSet *aContacts,
	...
	ActuatedModel_SDFast(aActuators,aContacts),_yi(0.0),_dpd(new DP_Data)
{
	setNull();
...
constructInitialStates()
{
	...
	// INITIALIZE
	_yi.setSize(ny);
	ActuatedModel_SDFast::getInitialStates(&_yi[0]);
	// q and u
	for(i=0;i<nqnu;i++) {
		_yi[i] = _dpd->y[i];
	}
...
applyActuatorForce(int aID)
{
	...
	// RD
	if(aID<na) {
		ActuatedModel_SDFast::applyActuatorForce(aID);
...
getActuatorForce(int aID) const
{
	...
	// RD
	if(aID<na) {
		force = ActuatedModel_SDFast::getActuatorForce(aID);
...
getActuatorStress(int aID) const
{
	...
	// RD
	if(aID<na) {
		stress = ActuatedModel_SDFast::getActuatorStress(aID);
...
getActuatorSpeed(int aID) const
{
	...
	// RD
	if(aID<na) {
		speed = ActuatedModel_SDFast::getActuatorSpeed(aID);
...
getActuatorPower(int aID) const 
{
	...
	// RD
	if(aID<na) {
		power = ActuatedModel_SDFast::getActuatorPower(aID);
 ...
 * to generic Model APIs to make it more general, however it's questionable
 ...
 * of mass are stored in the sdm structure as well as the Body objects. The sdm guy
...
scale(const ScaleSet& aScaleSet)
{
	...
	for(i=0; i < aScaleSet.getSize(); i++){
		...
		// or we can get index from BodySet and offset by 1 to account for ground 
		// not being a body.
	...
	// This must be done after the call to SDFast::scale as it assumes that mass, inertia 
	// and center of mass have been scaled already by SDFast::scale. The code below just synchronizes
	// SDFast and the sdm datastructure used by the pipeline to match the already scaled model.
	// I didn't make move sdfast calls to Body to have it Dynamics package agnostic.
