/*******************************************************************************
   ...
   Partial list of routines:
      ...
      integrate             : the variable step integrator used by main()
...
dpBoolean vectCompare(double a[3], double b[3])
{
	if (fabs(a[0] - b[0]) > SMALLNUM)
		return dpNo;
	if (fabs(a[1] - b[1]) > SMALLNUM)
		return dpNo;
	if (fabs(a[2] - b[2]) > SMALLNUM)
		return dpNo;
	return dpYes;
}
...
/* ADD_3x3MATRICES: add two 3x3 matrices */
void add_3x3matrices(double m1[][3], double m2[][3], double out[][3])
{
   int i, j;
                          ...
                          double velocity, double acceleration)
{
   ...
   else if (func->type == dpGCVSpline)
   {
      ...
      return splder(&deriv, &half_order, &func->numpoints, &abscissa, 
...
/* intersection between a line (pt1,pt2) and plane (plane1,d)
...
/* Determines if a 3D point is inside a 3D polygon. The polygon is the first
 * one in the PolyhedronStruct, and is assumed to be planar.
 */
...
/* Determines if a 2D point is inside a 2D polygon. Uses the "crossings test" taken from:
...
int polygon_ray_inter3d(dpPolyhedronStruct* newph,int poly_index, 
                        double pt[3],int axes)
{

   int i,j, num_inter, numpts;
   double **poly_pts, temppt[3], rotpt[3];
   dpPolygonStruct* poly;
   dpVertexStruct* v1;
   double **mat;
   double amat[3][3];
...
void find_plane_normal_to_line(dpPlaneStruct* plane, double pt1[], double pt2[])
{
