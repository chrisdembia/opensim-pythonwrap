				...
				char** output_motion_file, char** output_kinetics_file)
{
   ...
   while (1)
   {
      ...
      else if (STRINGS_ARE_EQUAL(buffer,"muscle_file") && muscle_file)
      {
         read_line(&fp, buffer);
         filename = clean_string(buffer);
         if (filename == NULL || strlen(filename) < 1)
...
ReturnCode read_kinetics_file(dpModelStruct* sdm, MotionData* data, char filename[])
{
   ...
   {
/*
      if (name_is_gencoord(data->elementnames[i], sdm, torque_suffix, &splineType, &cutoffFrequency) > 0)
         num_columns++;
*/
...
static ReturnCode verify_motion_data(dpModelStruct* sdm, MotionData* data)
{
   ...
   int i, gc, seg, musc, column, num_otherdata;
   dpSplineType splineType;
   double cutoffFrequency;
   ForceStruct* fs;
   TorqueStruct* ts;
...
static ReturnCode read_force_matte(FILE** fp)
{
   ...
   ReturnCode rc;
   dpPolyhedronStruct ph;
   char matte_name[CHARBUFFER], filename[CHARBUFFER], seg_name[CHARBUFFER];

   /* read matte name, file name, and segment name */
	if (fscanf(*fp,"%s %s %s", matte_name, filename, seg_name) != 3)
		sim_message(exit_program, "Error reading force_matte definition.");
