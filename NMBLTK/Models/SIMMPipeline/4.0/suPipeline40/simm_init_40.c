...
// EXTERNALS
//=============================================================================
char errorbuffer[1000]; // used locally to buffer error messages
...
int sduforce(double t, double q[], double u[])
{
   ...
   apply_muscle_forces(sdm);
	// Ayman: Added 0 for third parameter per SIMM4.0 changes.
    // This third parameter is not used unless a change in the source 
    // file pipelinetools.c is made
   apply_joint_restraint_torques(sdm, q, 0);
	...
	for (i = 0; i < sdm->nq; i++)
   {
      ...
      else
      {
         if (sdm->q[i].constraint_func == NULL)
         {
            ...
            sim_message(exit_program, errorbuffer);
         }
...
void assemble_model(double t, double y[])
{
   ...
   if (err)
   {
      ...
      sim_message(exit_program, buffer);
   }
...
void set_up_kinetics_input(char filename[], MotionData** data)
{
   ...
   {
      sim_message(exit_program,"Unable to malloc enough memory to run.\n");
   }
   ...
   */
    ...
    * of params.startTime, params.endTime, and params.stepSize to match the
