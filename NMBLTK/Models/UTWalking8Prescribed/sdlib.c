...
sdldudcomp(n,na,map,tol,ld,sum,m,l,d)
    int n,na,map[];
...
sdldubslv(n,na,map,work,l,d,b,x)
    int n,na,map[];
    double work[],l[],d[],b[],x[];
{
...
sdadjvars(func,vars,param,nfunc,ndes,dnorm,nvar,deltas,step,rerr,tderr,rtol,fcnt
      ,newvars,newerrs)
    int (*func)();
    double vars[],param[];
...
sdroot(func,vars,param,nfunc,nvar,ndesin,lock,rtol,dtol,maxeval,jw,dw,rw,iw,fret
      ,fcnt,err)
    int (*func)();
    double vars[],param[];
...
sdvinteg(func,time,st,dst,param,dt,step,neqin,tol,work,err,which)
    int (*func)();
...
sdprerrmsg(fnum,routine,errnum)
    FILE *fnum;
    int routine,errnum;
{
