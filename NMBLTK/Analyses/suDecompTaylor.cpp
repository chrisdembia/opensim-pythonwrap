...
constructDescription()
{
	printf("suDecompTaylor.constructDescription:\n");
	...
	void *aClientData)
{
	...
	for(c=0;c<nc;c++) {
		for(i=0;i<nu;i++) {
			I = rdMtx::ComputeIndex(c,nu,i,3,0);
			_ah[I+0] = 0.0;
			_ah[I+1] = 0.0;
			_ah[I+2] = 0.0;
		}
		...
		int aStep,double aDT,double aT,double *aX,double *aY)
{
	...
	for(i=0;i<np;i++) {
		...
		if(fabs(_sfrc[I])<=_contactThreshold) {
			...
			for(c=0;c<_nc;c++) {
				for(j=0;j<3;j++) {
					I = rdMtx::ComputeIndex(i,3,j);
					J = rdMtx::ComputeIndex(c,np,i,3,j);
		...
		// CONTACT JUST ESTABLISHED
			...
			J = rdMtx::ComputeIndex(_cVel,np,i,3,0);
			rdMtx::Add(1,3,fnv,ftv,&_fv[J]);
	...
	for(c=0;c<=getVelocityIndex();c++) {
		...
		for(i=0;i<np;i++) {
			bod = _model->getContactBodyB(i);
			_model->getContactPointB(i,pnt);
			I = rdMtx::ComputeIndex(c,np,i,3,0);
		...
		if(c==-1) {
			...
			for(i=0;i<np;i++) {
				I = rdMtx::ComputeIndex(c,np,i,3,1);
				printf("%lf ",_f[c][i][1]);
			}
			printf("\nVelocity:     ");
			for(i=0;i<np;i++) {
		...
		for(i=0;i<np;i++) {
			...
			rdMtx::Multiply(3,nu,1,_J,_du,dv);
 
			// VELOCITY BASED CHANGES IN VELOCITY
			if(c==getVelocityIndex()) {
				rdMtx::Subtract(3,nu,_J,_JPrev,_JDiff);
				rdMtx::Multiply(3,nu,1,_JDiff,_uPrev,dvTmp);
				rdMtx::Add(1,3,dv,dvTmp,dv);
			}

			// CHANGES IN POSITION
			for(j=0;j<3;j++) {
				I = rdMtx::ComputeIndex(c,np,i,3,j);
...
initializeForceElements()
{
	...
	char fileName[2048];
	double *feInit;
	rdStateVector *initVec=NULL;
	rdStorage *store=NULL;
	for(c=0;c<=getVelocityIndex();c++) {

		// LOAD DATA
		sprintf(fileName,"./DecompInit/fs_%s.xls",_cNames[c]);
		store = new rdStorage(fileName);
		if(store->getSize()<1) {
			printf("suDecompTaylor.integBeginCallback: WARN- failed to open ");
			printf("file %s\n",fileName);
...
computeFrictionFactors(double aT,double *aX,double *aY)
{
	...
	for(i=0;i<np;i++) {
		...
		//rdMtx::Add(1,3,&_sfrc[I],&_dfvFric[I],f);
		_model->getContactNormalForce(i,fnp,fnv,fn);
		_model->getContactTangentForce(i,ftp,ftv,ft);
		rdMtx::Add(1,3,fnv,ftv,fv);
		rdMtx::Add(1,3,fnp,ftp,fp);
		rdMtx::Add(1,3,fn,ft,f);
		rdMtx::Subtract(1,3,f,fp,f);

		//rdMtx::Add(1,3,fnp,fnv,fn);
		//rdMtx::Add(1,3,ftp,ftv,ft);
 ...
 * rdModel::computeContact() should be called.
 *
