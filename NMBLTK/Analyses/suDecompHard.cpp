 ...
 */
suDecompHard::suDecompHard(rdModel *aModel) :
	suDecomp(aModel)
{
	setNull();
		  ...
		  double *xt,double *y)
{
	...
	rdArray<double> dqdt(0.0,nq),dudt(0.0,nu);
	v3d *sacc = new v3d[np];
		...
		for(j=0;j<3;j++) {
			...
			if(whichC==0) {
				...
				*c = _bc[i].getPC(j)->evaluateC2(fsAcc);
				...
				printf("suDecompHard.suComputeConstraint: ERROR- invalid C\n");
			}
