 ...
 * Destructor.
 */
suDecompHardUTWalk::~suDecompHardUTWalk()
{
		  ...
		  double *xt,double *y)
{
	...
	suDecompTarget *decompTarget =
		new suDecompTarget(_nxs,getNumberConstraints(),this);
...
//	printf("suDecompHardUTWalk.determineConstraints: NC = %d\n",
		...
		for(j=0;j<3;j++) {
			...
			if(whichC==0) {
				...
				*c = _bc[i].getPC(j)->evaluateC2(fsAcc);
				...
				printf("suDecompHardUTWalk.suComputeConstraint: ERROR- invalid C\n");
			}
