 ...
 * Destructor.
 */
suActuatorPerturbationIndependent::~suActuatorPerturbationIndependent()
{
	deleteStorage();
}
//_____________________________________________________________________________
 ...
 */
void suActuatorPerturbationIndependent::
setNull()
{
	setType("suActuatorPerturbationIndependent");
	_step = 0;
	_unperturbedForceStorage = new rdStorage();
	_unperturbedForceStorage->setName("UnperturbedForces");
	_perturbedForceStorage = new rdStorage();
	_perturbedForceStorage->setName("PerturbedForces");
	constructDescription();
	constructColumnLabels();
	int na = _model->getNA();
	printf("suActuatorPerturbationIndependent.setNull: na = %d\n",na);
 ...
 */
bool suActuatorPerturbationIndependent::
 ...
 */
rdStorage* suActuatorPerturbationIndependent::
...
//	char name[rdObject::NAME_LENGTH];
	char name[500]; // needs to be changed to [rdObject::NAME LENGTH]
 ...
 */
int suActuatorPerturbationIndependent::
getStep()
{
	return(_step);
}
...
computeActuation(double aT,double *aX,double *aY)
{
	...
	//printf("suActuatorPerturbationIndependent.computeActuation: aT=%.16lf  aY[0]=%.16lf\n",aT,aY[0]);

	if(_model==NULL) {
		printf("suActuatorPerturbation.computeActuation: WARN- no model.\n");
		return;
	}
	if(!getOn()) return;
	...
	if(!_recordUnperturbedForces) {
		...
		if((aT>=getStartTime()) && (aT<=getEndTime())){
			if( _perturbationType == SCALE)	{
				...
				printf("suActuatorPerturbationIndependent.computeActuation:WARN - unrecognized perturbationType.\n");
				force = _force;
			}
...
applyActuation(double aT,double *aX,double *aY)
{
 //  printf("\t\tapplyActuation: aT=%.16lf  aY[0]=%.16lf\n",aT,aY[0]);
	if(_model==NULL) {
		printf("suActuatorPerturbationIndependent.applyActuation: WARN- no model.\n");
		return;
	}
	if(!getOn()) return;
