...
//_____________________________________________________________________________
/**
 * Construct an suContact object for recording the various qunatities
 ...
 *  model.
 */
suContact::suContact(rdModel *aModel,int *aResultantForcePointGroups) :
	rdAnalysis(aModel)
{
	// NULL
	setNull();
...
record(double aT,double *aX,double *aY)
{
	...
	for(p=0;p<np;p++) {
		...
		rdMtx::Assign(1,3,vec,&_forces[I+3]);

		// FORCE ON BodyA IN GROUND FRAME
		rdMtx::Multiply(1,3,vec,-1.0,&_forces[I]);
	}
	...
	for(g=0;g<_nResultantForcePointGroups;g++){
		...
		for(p=0;p<np;p++){
			if(_resultantForcePointGroupAssignements[p]==g){
				...
				rdMtx::Add(1,3,totContactForce,contactForce,totContactForce);
...
//				rdMtx::Subtract(1,3,contactPosGlobal,posBodyCOMGlobal,contactPosRelCOMGlobal);

				rdMtx::CrossProduct(contactPosRelCOMGlobal,contactForce,contactTorque);
				rdMtx::Add(1,3,contactTorque,totContactTorque,totContactTorque);
			}
		}
		...
		for(j=0;j<3;j++){
			index = rdMtx::ComputeIndex(g,3,j);
...
{
	if(!getOn()) {
		printf("suContact.printResults: Off- not printing.\n");
		return(-1);
	}
