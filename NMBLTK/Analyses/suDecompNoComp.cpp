...
// NAMES
const char *suDecompNoComp::ADDON_COMPONENT_NAMES[] = {
 ...
 */
suDecompNoComp::suDecompNoComp(rdModel *aModel) :
	rdAnalysis(aModel)
{
	int i;
 ...
 */
suDecompNoComp::suDecompNoComp(rdModel *aModel,char *aBaseName,char *aDir,char *aExtension) :
	rdAnalysis(aModel)
{
	printf("suDecompNoComp: constructing induced acceleration analysis from file.\n");
...
constructDescription()
{
	printf("suDecompNoComp.constructDescription:\n");
...
setContactEstablished(int aIndex,bool aTrueFalse)
{
	if((aIndex<0)||(aIndex>=_model->getNP())) {
		printf("suDecompNoComp.setContactEstablished: WARN- index out of range.\n");
		return;
	}
...
{
	if((aIndex<0)||(aIndex>=_model->getNP())) {
		printf("suDecompNoComp.getContactEstablished: WARN- index out of range.\n");
		return(false);
	}
 ...
 */
const rdStorage* suDecompNoComp::
...
{
	if(!proceed()) return(0);
	printf("suDecompNoComp.end: Finalizing analysis %s.\n",getName().c_str());
	return(0);
}
...
{
	if(!getOn()) {
		printf("suDecompNoComp.printResults: Off- not printing.\n");
		return(-1);
	}
	...
	if(_fStore[c]!=NULL){
		...
		// FILE NAME
		if(aExtension==NULL) {
			sprintf(name,"%s/%s_%s_noComp",path,baseName,getName().c_str());	
		} else {
			sprintf(name,"%s/%s_%s_noComp%s",path,baseName,getName().c_str(),
			aExtension);
		}
	...
	if(_cpaStore!=NULL) {
		...
		// FILE NAME
		if(aExtension==NULL) {
			sprintf(name,"%s/%s_%s_cpAccel_noComp",path,baseName,getName().c_str());
		} else {
			sprintf(name,"%s/%s_%s_cpAccel_noComp%s",path,baseName,getName().c_str(),
				aExtension);
		}
