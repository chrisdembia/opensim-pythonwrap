...
// CONSTANTS
//=============================================================================
const int suBodyPointIndAcc::NAME_LENGTH = suBodyPointIndAcc_NAME_LENGTH;
const int suBodyPointIndAcc::BUFFER_LENGTH = suBodyPointIndAcc_BUFFER_LENGTH;
 ...
 * Destructor.
 */
suBodyPointIndAcc::~suBodyPointIndAcc()
{
	// STORAGE
	deletePointStorage();
}
...
suBodyPointIndAcc::
suBodyPointIndAcc(rdModel *aModel,int aBody,double aPoint[3],
	rdStorage *aStates,rdStorage *aControls,char *aBaseName,char *aDir,char *aExtension) :
	suIndAcc(aModel,aStates,aControls,aBaseName,aDir,aExtension)
{
	printf("suBodyPointIndAcc: constructing analysis from file.\n");
...
allocatePointStorage()
{
	// ALLOCATE STORAGE
	// X
	_axPointStore = new rdStorage(500,"BodyPointInducedAccelerations_X");
	...
	// Y
	_ayPointStore = new rdStorage(500,"BodyPointInducedAccelerations_Y");
	...
	// Z
	_azPointStore = new rdStorage(500,"BodyPointInducedAccelerations_Z");
...
setBody(int aBody)
{
	if((aBody<0)||(aBody>=_model->getNB())) {
		printf("suBodyPointIndAcc.setBody:  WARN- invalid body ID %d.\n",aBody);
...
computePointAccelerations()
{
	...
	if(!getUseNullDecomposition()) {
		...
		if((ti!=_yStore->getFirstTime())||(tf!=_yStore->getLastTime())) {
			printf("suBodyPointIndAcc.computeAccelerations: WARN-\n");
	...
	for(i=0;i<_yStore->getSize();i++) {
		...
		I = rdMtx::ComputeIndex(getAllIndex(),3,0);
	...
	// INDUCED POSITION DUE TO INITIAL POSITION AND INITIAL VELOCITY
		...
		_iVelStore = new rdStorage(500,"InitialPointVelocityX");		
		...
		_iPosStore = new rdStorage(500,"InducedPointPositionDueToInitVel&Pos");	
