 ...
 * Destructor.
 */
suBodyIndAccCOM::~suBodyIndAccCOM()
{
	// STORAGE
 ...
 */
suBodyIndAccCOM::suBodyIndAccCOM(rdModel *aModel, int aN,
	 int aBodyList[]) : suBodyIndAcc(aModel)
{
 ...
 */
suBodyIndAccCOM::suBodyIndAccCOM(rdModel *aModel,rdStorage *aStates,
	rdStorage *aControls,char *aBaseName,char *aDir,char *aExtension,
	int aN,int aBodyList[]) :
	suBodyIndAcc(aModel,aStates,aControls,aBaseName,aDir,aExtension)
{
	printf("suBodyIndAccCOM: constructing COM induced acceleration analysis from file.\n");
...
allocateBodyStorage()
{
	_aeCOMBodyStore = new rdStorage(500,"BodyCOMInducedAccelerations");
	...
	_peCOMBodyStore = new rdStorage(500,"BodyCOMInducedPositions");
	_veCOMBodyStore = new rdStorage(500,"BodyCOMInducedVelocities");
	
	_posStore = new rdStorage(500,"BodyCOMPosition");
	_velStore = new rdStorage(500,"BodyCOMVelocity");
...
computeBodyCOMAccelerations()
{
	...
	double totMass = 0.0;
	double time = 0.0;
	rdStorage *dataStorage = NULL;
	rdStateVector *stateVec = NULL;
	rdStateVector *stateStoreVec = NULL;
	double *dataVec = NULL;
	...
	for(t=0;t<_yStore->getSize();t++) {
		...
		for(c=0;c<nc;c++){
			...
			for(body=0;body<_aN;body++) {
				...
				// COMPUTE
				for(int k=0;k<3;k++){	
					I = rdMtx::ComputeIndex(_aBodyList[body],6,k);
					J = rdMtx::ComputeIndex(c,3,k);
		...
		if(_aeCOMBodyStore==NULL) {
			_aeCOMBodyStore = new
				rdStorage(1000,"COMBodyInducedAccelerations");
			_aeCOMBodyStore->setDescription(getDescription());
			_aeCOMBodyStore->setColumnLabels(getColumnLabels());
		}			
	...
	// INDUCED POSITION DUE TO INITIAL POSITION AND INITIAL VELOCITY
		...
		_iVelStore = new rdStorage(*_velStore, false);
		...
		_iPosStore = new rdStorage(*_posStore, false);
