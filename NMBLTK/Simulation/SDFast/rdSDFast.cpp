...
//_____________________________________________________________________________
/**
 * Assemble the model by calling SDFast code to perform assembly.
...
scale(const suScaleSet& aScaleSet)
{
	...
	for(i=0; i < aScaleSet.getSize(); i++){
		...
		rdBody* segment = getBodySet()->get(segName);
...
dump(const std::string& aFileName)
{
	...
	for (int i=0; i < _nb;i++) {
		...
		rdBody* segment = getBodySet()->get(i);
		double com[3];
...
constructBodies()
{
	rdModel::constructBodies();

	for(int i=0; i < _nb; i++){
		rdBody* bdy = getBodySet()->get(i);
...
transformPosition(int aBody,const double aPos[3],double rPos[3]) const
{
   ...
   double posBodyCOMLocal[3] = {0,0,0};
	...
	rdMtx::Subtract(1,3,posGlobal,posBodyCOMGlobal,rPos);
...
{
	sdderiv(dqdt,dudt);
	rdMtx::Assign(1,getNU(),dudt,_dudt);
	return(0);
}
	...
	int aRefBody) const
{
	...
	for(i=0;i<getNU();i++) {
		...
		// FORM MATRIX
		for(j=0;j<3;j++) {
			I = rdMtx::ComputeIndex(j,_u.getSize(),i);
			rJ[I] = trans[j];
		}
	}
...
formJacobianEuler(int aBody,double *rJE,int aRefBody) const
{
	...
	// TRANSFORM J0 TO JE
	rdMtx::Multiply(3,3,getNU(),&E[0][0],rJE,rJE);
...
/**
 * Convert the rotational generalized coordinates or speeds from units of
 * radians to units of degrees for all the state-vectors in an rdStorage
 * object.
 *
 * It is assumed that the first first getNU() elements of each state-vector
 * held in the rdStorage object are the generalized coordinates or speeds.
 *
...
/**
 * Convert the rotational generalized coordinates or speeds from units of
 * degrees to units of radians for all the state-vectors in an rdStorage
 * object.
 *
 * It is assumed that the first first getNU() elements of each state-vector
 * held in the rdStorage object are the generalized coordinates or speeds.
 ...
 *
 * This static method is intended to be called from the SDFast function
 * sduforce().  sduforce() is a "C" function and, therefore, doesn't know
 * about C++ objects.  Internally, this method uses a static pointer
 * to call the virtual methods rdModel::applyActuaion() and
 * rdModel::applyContact().  This method assumes that computeActuation()
 * and computeContact() have already been called.
 */
void rdSDFast::
SDUForce()
{
	cout<<"\n\nrdSDFast::SDUForce: ... SDFast has called sudforce()!\n\n";

	if(_Instance==NULL) {
		cout<<"\n\nrdSDFast.SDUForce: ERR- no valid rdSDFast object.\n\n";
		return;
	}
