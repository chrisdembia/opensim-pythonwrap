...
JointMoment(int aQID,int aNX,int aNY,int aNYP) :
	rdGeneralizedForceAtv(aQID,aNX,aNY,aNYP),
...
JointMoment(DOMElement *aElement,int aNX,int aNY,int aNYP) :
	rdGeneralizedForceAtv(aElement,aNX,aNY,aNYP),
	_optimalNegForce(_propOptimalNegForce.getValueDbl())
{
	setNull();
	updateFromXMLNode();
...
JointMoment(const JointMoment &aActuator) :
	rdGeneralizedForceAtv(aActuator),
	_optimalNegForce(_propOptimalNegForce.getValueDbl())
{
	setNull();
	copyData(aActuator);
 ...
 */
rdObject* JointMoment::
copy() const
{
	rdActuator *act = new JointMoment(*this);
	return(act);
}
//_____________________________________________________________________________
/**
...
{
	// BASE CLASS
	rdGeneralizedForceAtv::operator =(aActuator);
...
updateFromXMLNode()
{
	rdGeneralizedForceAtv::updateFromXMLNode();
