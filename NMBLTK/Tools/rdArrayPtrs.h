...
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#ifndef __rdArrayPtrs_h__
#define __rdArrayPtrs_h__
 ...
 *
 * In constrast to class rdArray<T>, when an object is added to this array
 * a copy is not made.  Rather, a pointer to the added object is
 * stored in the array.
 *
 * When an rdArrayPtrs object falls out of scope or is deleted, all objects
 ...
 */
template<class T> class rdArrayPtrs
{
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// DATA
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
protected:
	/** Flag indicating whether this rdArrayPtrs object is responsible for
 ...
 */
rdArrayPtrs(const rdArrayPtrs<T> &aArray)
{
	setNull();
	*this = aArray;
}
 ...
 */
#ifndef SWIG
rdArrayPtrs<T>& operator=(const rdArrayPtrs<T> &aArray)
{
	// DELETE OLD ARRAY
	if(_memoryOwner) clearAndDestroy();
 ...
 * the array.  If error checking is desired, use rdArrayPtrs::get().
 *
 * @param aIndex Index of the desired element (0 <= aIndex < _size).
 * @return Reference to the array element.
 * @throws rdException if a NULL pointer is encountered.
 * @see get().
 */
 ...
 *
 * rdArrayPtrs[size] = T[0] T[1] T[2] ... T[size-1].
 *
 * @param aOut Output stream.
 * @param aArray Array to be output.
 * @return Reference to the output stream.
 */
friend std::ostream& operator<<(std::ostream &aOut,const rdArrayPtrs<T> &aArray)
{
	aOut << "rdArrayPtrs[" << aArray.getSize() <<"] =";
...
bool computeNewCapacity(int aMinCapacity,int &rNewCapacity)
{
	...
	if(_capacityIncrement == 0) {
		printf("rdArrayPtrs.computeNewCapacity: WARN- capacity is set");
...
bool ensureCapacity(int aCapacity)
{
	...
	if(newArray==NULL) {
		printf("rdArrayPtrs.ensureCapacity: ERR- failed to increase capacity.\n");
		return(false);
	}
...
void trim()
{
	...
	if(_array==NULL) {
		printf("rdArrayPtrs.trim: ERR- unable to allocate array.\n");
		return;
	}
...
{
	if(aObject==NULL) {
		std::cout<<"rdArrayPtrs.append: ERR- NULL pointer."<<std::endl;
		return(false);
	}
 ...
 * @return True if the append was successful, false otherwise.
 */
bool append(rdArrayPtrs<T> &aArray)
{
...
bool insert(int aIndex,T *aObject)
{
	// NULL POINTER
	if(aObject==NULL) {
		std::cout<<"rdArrayPtrs.insert: ERR- NULL pointer."<<std::endl;
		return(false);
	}

	// NEGATIVE INDEX
	if(aIndex<0) {
		printf("rdArrayPtrs.insert: ERR- aIndex was less than 0.\n");
		return(false);
	}
 ...
 * @throws rdException if (aIndex<0)||(aIndex>=_size) or if the pointer
 * at aIndex is NULL.
 * @see operator[].
 */
T* get(int aIndex)
{
	if((aIndex<0)||(aIndex>=_size)) {
		throw(rdException("rdArrayPtrs.get: Array index out of bounds."));
	}
	if(_array[aIndex]==NULL) {
		throw(rdException("rdArrayPtrs.get: NULL pointer.",
			__FILE__,__LINE__));
	}
 ...
 * @throws rdException if no such object exists.
 * @see getIndex()
 */
T* get(const std::string &aName)
{
	...
	if(index==-1) {
		std::string msg = "rdArrayPtrs.get(aName): No object with name ";
		msg += aName;
 ...
 * @throws rdException if the array is empty.
 */
T* getLast() const
{
	if(_size<=0) {
...
//=============================================================================
};	// END of class rdArrayPtrs
//=============================================================================
//=============================================================================


#endif //__rdArrayPtrs_h__
