...
double *sigr;
	...
	sigr = new double[N];
	if(sigr==NULL) {
		printf("\nrdSignal.lowpassIIR: ERROR- Not enough memory.\n");
		return(-1);
	}
...
Pad(int aPad,int aN,const double aSignal[])
{
	...
	if (s == NULL) {
		printf("\n\nrdSignal.Pad: Failed to allocate memory.\n");
		return(NULL);
	}
							...
							rdArray<double> &rTime,rdArray<double> &rSignal)
{
	...
	if(size!=sizeSignal) {
		cout<<"\n\nrdSignal.ReduceNumberOfPoints:: quitting.  The time and signal ";
	...
	// APPEND FIRST POINT
	rdArray<double> t(0.0,0,size),s(0.0,0,size);
	t.append(rSignal[0]);
	s.append(rSignal[0]);
	int iLast=0;
	...
	for(i=1;i<(size-1);i++) {
		...
		rdMtx::Subtract(1,3,p2,p1,v1);
		rdMtx::Subtract(1,3,p3,p1,v2);

		mv1 = rdMtx::Magnitude(3,v1);
		mv2 = rdMtx::Magnitude(3,v2);
		cos = rdMtx::DotProduct(3,v1,v2) / (mv1*mv2); 
