...
// CONSTANTS
const int rdXMLNode::TABLIMIT = 128;
 ...
 * @throws rdException if it was not possible to get a value.
 */
bool rdXMLNode::
GetBool(const DOMNode *aNode)
{
	if(aNode==NULL) {
		string msg = "rdXMLNode.getBool: XML node is NULL.";
	...
	if(textNode==NULL) {
		string msg = "rdXMLNode.getBool: XML node has no text node.";
		throw( rdException(msg,__FILE__,__LINE__) );
	}
	...
	if(str==NULL) {
		string msg = "rdXMLNode.getBool: XML text node is NULL.";
		throw( rdException(msg,__FILE__,__LINE__) );
	}
...
{
	if(aNode==NULL) {
		string msg = "rdXMLNode.getInt: XML node is NULL.";
		throw( rdException(msg,__FILE__,__LINE__) );
	}
	...
	if(status!=1) {
		...
		string msg = "rdXMLNode.getInt: ERROR- failed to interpret value of ";
		msg += name;
		msg += " as an int.";
		if(str!=NULL) { delete[] str; }
		throw( rdException(msg,__FILE__,__LINE__) );
...
{
	if(aNode==NULL) {
		string msg = "rdXMLNode.getDbl: XML node is NULL.";
		throw( rdException(msg,__FILE__,__LINE__) );
	}
	...
	if(status!=1)  {
		string name = XMLString::transcode(aNode->getNodeName());
		string msg = "rdXMLNode.getDbl: ERROR- failed to interpret value of ";
		msg += name;
		msg += " as a double.";
		if(str!=NULL) { delete[] str; }
		throw( rdException(msg,__FILE__,__LINE__) );
...
{
	if(aNode==NULL) {
		string msg = "rdXMLNode.getStr: XML node is NULL.";
		throw( rdException(msg,__FILE__,__LINE__) );
	}
...
SetBoolArray(DOMNode *aNode,int aN,const bool *aData)
{
	...
	if(textNode==NULL) {
		printf("rdXMLNode.SetIntArray: ERROR- unable to find or create ");
		printf("text node.\n");
...
SetDblArray(DOMNode *aNode,int aN,const double *aData)
{
	...
	// CHECK CharacterData NODE
	if(textNode==NULL) {
		printf("rdXMLNode.SetDblArray: ERROR- unable to find or create ");
		printf("text node.\n");
		if(empty!=NULL) delete[] empty;
		if(space!=NULL) delete[] space;
		return;
...
SetStrArray(DOMNode *aNode,int aN,char **aData)
{
	...
	// CHECK CharacterData NODE
	if(textNode==NULL) {
		printf("rdXMLNode.SetStrArray: ERROR- unable to find or create ");
		printf("text node.\n");
		if(empty!=NULL) delete[] empty;
		if(space!=NULL) delete[] space;
		return;
