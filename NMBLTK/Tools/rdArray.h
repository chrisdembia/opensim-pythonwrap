 ...
 * the array.  If error checking is desired, use rdArray::get().
 *
 ...
 */
bool operator==(const rdArray<T> &aArray) const
{
 ...
 *
 * rdArray[size] = T[0] T[1] T[2] ... T[size-1].
 *
 * @param aOut Output stream.
 * @param aArray Array to be output.
 * @return Reference to the output stream.
 */
friend std::ostream& operator<<(std::ostream &aOut,const rdArray<T> &aArray)
{
	aOut << "rdArray["<<aArray.getSize()<<"]=";
 ...
 * @return Reference to the array element.
 * @throws rdException if (aIndex<0)||(aIndex>=_size).
 * @see operator[].
 */
