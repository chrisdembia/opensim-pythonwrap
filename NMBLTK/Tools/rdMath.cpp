 ...
 */
int rdMath::
ComputeIntersection(const rdLine *aLine,const rdPlane *aPlane,double rPoint[3])
{
	...
	double denom = rdMtx::DotProduct(3,direction,normal);

	// NUMERATOR- DOT OF PLANE NORMAL WITH DIFFERENCE OF PLANE AND LINE POINTS
	double lo[3],po[3],lopo[3];
	aLine->getPoint(lo);
	aPlane->getPoint(po);
	rdMtx::Subtract(1,3,po,lo,lopo);
	double numer = rdMtx::DotProduct(3,normal,lopo);
				...
				double rNormal[3])
{
	...
	// CROSS PRODUCT
	rdMtx::CrossProduct(p12,p23,rNormal);

	// POINT
	double mag = rdMtx::Normalize(3,rNormal,rNormal);

	// ERROR CHECK
	if(mag<rdMath::ZERO) {
		printf("rdPlane.ComputeNormal: WARN- normal is zero or very small.\n");
