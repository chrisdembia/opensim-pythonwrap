 ...
 * Table model for object review dialog. Maps properties from an rdObject to a
...
public class SimtkObjectTableModel extends AbstractTableModel {
  ...
  public SimtkObjectTableModel(rdObject aObject) {
    _aObject = aObject;
    buildTable();
  }
  ...
  private void buildTable() {
    rdPropertySet objectProps = _aObject.getPropertySet();
    ...
    for (int i = 0; i < objectProps.getSize(); i++) {
      rdProperty prop = objectProps.get(i);
      _rowLabels[i] = prop.getName();
         if(prop.getType()== rdProperty.Int)
          _rowValues[i] = String.valueOf(prop.getValueInt());
        else if(prop.getType()== rdProperty.Dbl)
