...
/* Probably not the best way to define errno for Cygwin,
 * but I can't figure out the proper use of the system header files.
 * JPL 02/18/05
 */
...
int acpp_main(int argc, char** argv, _msg_proc msg)
{
  ...
  if (print_deps == 0
      ...
      if (*s != 0)
	{
	  ...
	  bcopy_gcc (spec, output_file, s - spec);
     ...
     FILE_BUF *buf;
{
  ...
  while ((sptr = (U_CHAR *) index (sptr, '?')) != NULL) {
    ...
    len = sptr - fptr - 2;
    if (bptr != fptr && len > 0)
      bcopy_gcc (fptr, bptr, len);	/* BSD doc says bcopy_gcc () works right
                ...
                }
                {
                   ...
                   while (ibp < limit) {
                      if (*ibp++ == '\n') {
                         ibp--;
                         if (put_out_comments) {
                            bcopy_gcc (before_bp, obp, ibp - before_bp);
                            obp += ibp - before_bp;
                         }
                         break;
                      }
  ...
  for (kt = directive_table; kt->length > 0; kt++) {
	...
	*cp = 0;
	...
	/* Output directive name.  */
        ...
        *op->bufp++ = '#';
        bcopy_gcc (kt->name, op->bufp, kt->length);
        op->bufp += kt->length;

	/* Output arguments.  */
        len = (cp - buf);
        check_expand (op, len);
        bcopy_gcc (buf, op->bufp, len);
        op->bufp += len;
      }
 ...
 */
static void error (msg, arg1, arg2, arg3)
     char *msg;
{
  int i;
  FILE_BUF *ip = NULL;
...
get_filename:
  ...
  fail:
    if (retried) {
      ...
      bcopy_gcc (trybuf.buf, buf, trybuf.bufp - trybuf.buf);
     ...
     FILE_BUF *op;
{
  ...
  if (st_mode & S_IFREG) {
    ...
    fp->buf = (U_CHAR *) alloca (st_size + 2);
    fp->bufp = fp->buf;
    bcopy_gcc (basep, fp->buf, st_size);
    fp->length = st_size;
    free (basep);
  }
     ...
     struct directive *keyword;
{
 ...
 dm_add_define(symname,bp - symname, bp,limit - bp,ip->lineno);
	...
	bcopy_gcc (temp->name, &defn->argnames[i], temp->length);
	i += temp->length;
	if (temp->next != 0) {
	  defn->argnames[i++] = ',';
	  defn->argnames[i++] = ' ';
...

  {
     ...
     if ((hp = lookup (symname, sym_length, hashcode)) != NULL)
     {
        if (hp->type != T_MACRO || compare_defs (defn, hp->value.defn))
        {
              ...
              bcopy_gcc (symname, msg, sym_length);
  ...
  if (*bp == '\"') {
    ...
    if (hp == 0) {
      ...
      *hash_bucket = hp;
      ...
      bcopy_gcc (fname, hp->value.cpval, fname_length);
    }
  } else if (*bp) {
    error ("invalid format #line command");
    return 1;
     ...
     enum file_change_code file_change;
{
  ...
  if (op->bufp > op->buf && op->bufp[-1] != '\n')
    *op->bufp++ = '\n';
  bcopy_gcc (line_cmd_buf, op->bufp, len);
  op->bufp += len;
  op->lineno = ip->lineno;
}

     ...
     FILE_BUF *op;
{
  ...
  if (nargs >= 0) {
	...
	if (ap->stringify != 0) {
	  ...
	  bcopy_gcc (p1, xbuf + totlen, l1 - p1);
	  totlen += l1 - p1;
	} else {
	  bcopy_gcc (arg->expanded, xbuf + totlen, arg->expand_length);
     ...
     register struct argdata *argptr;
{
  ...
  if (!(ip->fname != 0 && (newlines != 0 || comments != 0))
    ...
    bcopy_gcc (ip->bufp, buffer, bufsize);
    ip->bufp = bp;
    ip->lineno += newlines;
	...
	return "unterminated macro call";
      ...
      bcopy_gcc (ip->bufp, buffer + bufsize - (bp - ip->bufp), bp - ip->bufp);
      ip->bufp = bp;
      ip->lineno += newlines;
    }
...
#define MAX_OUTPUT_COLUMNS 75
  ...
  bcopy_gcc (string, &deps_buffer[deps_size], size);
  deps_size += size;
  deps_column += size;
