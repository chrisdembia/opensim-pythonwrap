/*******************************************************************************
   ...
   Description: This file contains functions that implement various
   ...
   Functions:
      make_vector               : makes a 1x3 vector
      find_end_of_array         : finds the end of an integer array
      compute_angle_between_vectors : calcs angle between two vectors
      cross_vectors             : finds cros product of two vectors
      normalize_vector          : normalizes a vector
      clear_vector              : sets all vector elements to 0.0
      transform_pt              : multiplies a point by a transform matrix
      transform_vec             : multiplies a vector by a transform matrix
      vect_equal_within_tol     : checks if two vectors are equal
      make_3x3matrix            : makes a 3x3 matrix
      mult_3x3matrix_by_vector  : premultiplies a vector by a 3x3 matrix
      make_3x3dircos_matrix     : makes a 3x3 direction cosine matrix from axis/angle
      reset_3x3matrix           : sets a 3x3 matrix to the identity matrix
      make_3x3_xrot_matrix      : makes a 3x3 direction cosine matrix with X rotation
      mult_3x3matrices          : multiplies two 3x3 matrices
      transpose_3x3matrix       : transposes a 3x3 matrix
      copy_3x3matrix            : copies a 3x3 matrix
      mult_3x3matrix_by_vector  : multiplies a 1x3 vector by a 3x3 matrix
      zero_3x3matrix            : checks if any row of a 3x3 matrix is all zeros
      mult_4x4matrix_by_vector  : multiplies a 1x4 vector by a 4x4 matrix
      mult_4x4matrices          : multiplies two 4x4 matrices
      make_4x4dircos_matrix     : makes a 4x4 direction cosine matrix from axis/angle
      translate_matrix          : appends a translation onto a 4x4 matrix
      append_matrix             : multiplies two 4x4 matrices and stores result in first
      ...
      intersect_line_plane01    : calcs intersection between line segment and plane
      intersect_line_plane      : calcs intersection between line and plane
      point_in_polygon3D        : determines if a point is inside a 3D polygon
      point_in_polygon2D        : determines if a point is inside a 2D polygon
      intersect_lines           : calcs intersection of two lines in 3D
      intersect_lines_scaled    : alternate form of intersect_lines()
      project_point_onto_plane  : projects 3D point onto 3D plane
      distancesqr_between_vertices : calcs square of distance between two 3D points
      distance_between_vertices : calcs distance between two 3D points
      get_distsqr_point_line    : calcs square of shortest distance from point to plane
      get_point_from_point_line : calcs point on line closest to a 3D point
      quat_to_axis_angle_rot    : extracts angle and axis from a quaternion
      matrix_to_quat            : converts a 4x4 transform matrix to a quaternion
      extract_rotation          : extracts angle and axis from a 4x4 transform matrix
      extract_xyz_rot_spacefixed : extracts XYZ space-fixed rotations from matrix
      extract_xyz_rot_bodyfixed : extracts XYZ body-fixed rotations from matrix
      make_quaternion           : makes a quaternion given an angle and axis
      quat_to_matrix            : converts a quaternion into a 4x4 transform matrix
      rotate_matrix_by_quat     : appends a quaternion rotation onto a 4x4 matrix
      rotate_matrix_axis_angle  : appends an angle/axis rotation onto a 4x4 matrix
      format_double             : determines appropriate printf format for a number
      print_3x3matrix           : prints a 3x3 matrix to a file
      integrate                 : variable step integrator used by main() and dllmain()
...
/* MAKE_VECTOR: stores X,Y,Z values in an array. */
...
/* COMPUTE_ANGLE_BETWEEN_VECTORS: calculates angle between two vectors.
 * Result is in the range 0 to pi radians.
 */
...
/* MAKE_3X3MATRIX: makes a 3x3 matrix, given the 9 elements. */
...
/* MULT_3X3MATRIX_BY_VECTOR: premultiplies a 3x3 matrix by
...
/* MAKE_3X3DIRCOS_MATRIX: makes a 3x3 direction cosine matrix from an
 * angle and an axis.
 */
...
/* MAKE_3X3_XROT_MATRIX: makes a 3x3 direction cosine matrix with
...
/* MULT_3X3_BY_VECTOR: multiplies a 1x3 vector by a 3x3 matrix. */
...
/* ZERO_3X3MATRIX: checks if any row of a 3x3 matrix contains all zeros. */
...
/* APPEND_MATRIX: computes m * b and stores the result in m. */
                          ...
                          double velocity, double acceleration)
{
   ...
   if (func == NULL)
      sim_message(exit_program, "interpolate_spline: attempted to access NULL function.");
...
/* INTERSECT_LINE_PLANE01: calculates intersection between a line segment
 * (pt1,pt2) and plane (plane1,d). Also returns the intersection point(inter)
 * and where on the line it is(t). Ratio t must lie between 0 and 1 or no
...
/* INTERSECT_LINE_PLANE: calculates intersection between a line (pt1,pt2)
 * and plane (plane1,d). Also returns the intersection point(inter) and
 * where on the line it is(t). Ratio t does not have to be between 0 and 1.
 */
...
dpBoolean point_in_polygon3D(double pt[], dpPolyhedronStruct* ph, int polygon_index)
{
    ...
    */
   skip_coord = XX;
   max_value = ph->polygon[polygon_index].normal[XX];
   if (DOUBLE_ABS(ph->polygon[polygon_index].normal[YY] > max_value))
   {
      skip_coord = YY;
      max_value = ph->polygon[polygon_index].normal[YY];
   }
   if (DOUBLE_ABS(ph->polygon[polygon_index].normal[ZZ] > max_value))
      skip_coord = ZZ;
...
/* INTERSECT_LINES: calculates the intersection of two lines. Returns where
 * on line1 the intersection is (s) and where on line 2 (t). Scales s and t
...
/* INTERSECT_LINES_SCALED: same as intersect_lines(), but does not scale s and t
 * to be between 0.0 and 1.0. S ranges from 0.0 to mag2, and T ranges from 0.0 to mag1.
 */
...
/* PROJECT_POINT_ONTO_PLANE: projects a 3D point onto the surface of a 3D plane. */
...
/* DISTANCESQR_BETWEEN_VERTICES: calculates the square of the distance
...
/* DISTANCE_BETWEEN_VERTICES: calculates the distance between two 3D points. */
...
/* GET_DISTSQR_POINT_LINE: calculates the square of the shortest distance from
 * a point (point) to a line (vl, through pl).
 */
...
/* GET_POINT_FROM_POINT_LINE: calculates the closest 3d point to given 3d line.
 * The line is defined as vector(vec) and a point(pt) and has not been normalized
 * to a unit vector. The value hypo*(cosalpha) of a rt triangle is calculated
...
/* QUAT_TO_AXIS_ANGLE_ROT: extract angle and axis values from a quaternion. */
...
/* MATRIX_TO_QUAT: converts a 4x4 transform matrix to an equivalent quaternion. */
...
/* EXTRACT_XYZ_ROT_SPACEFIXED: extracts XYZ space-fixed rotations
 * from a 4x4 transform matrix.
 */
...
/* EXTRACT_XYZ_ROT_BODYFIXED: extracts XYZ body-fixed rotations
 * from a 4x4 transform matrix.
 */
...
/* QUAT_TO_MATRIX: converts a quaternion into an equivalent
...
/* ROTATE_MATRIX_BY_QUAT: appends a quaternion rotation onto a 4x4 transform matrix. */
...
/* ROTATE_MATRIX_AXIS_ANGLE: appends an angle/axis rotation onto a 4x4 transform matrix. */
...
/* A variable-step integrator, taken from the SD/FAST-generated file sdlib.c.
 * Work should be dimensioned 6*neq.
 */

void integrate(void (*func)(),
    double *time,
    double st[],
    double dst[],
    double param[],
    ...
    int *which)
{
    ...
    *err = *which = 0;
    ...
    while (ttime  <  tfin)
    {
        ...
        {
            lookingForImpact = NO;
            sdrk4m(func,ttime,st,dst,param,nstp,&work[xst],neq,&work[morework],&work[errs],&maxerr,&wh);
            xtime = ttime + nstp;
            if (maxerr <= tol)
            {
                func(xtime,&work[xst],&work[xdst],param,&errf);
                if (errf == 1)
                   lookingForImpact = YES;
                else
                   lookingForImpact = NO;
                if (errf == 0)
                {
                   stp2 = 2.0 * nstp;
                   if ((maxerr * 64.0 <= tol) && (stp2 <= dt))
                   {
                        stp = stp2;
                   }
                   else
            ...
            if (lookingForImpact == YES && stp2 < MIN_IMPACT_STEP)
            {
               *err = 3;
               *which = IMPACT_FLAG;
               *time = xtime;
