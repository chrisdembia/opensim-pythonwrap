...
{
   int seg, obj_num, count;
   ReturnCode rc;
   dpBodyStruct* body;
   dpPolyhedronStruct ph;
   dpPolyhedronStruct* ph_ptr;
   char *clean_name, obj_name[CHARBUFFER], filename[CHARBUFFER], seg_name[CHARBUFFER];

	/* Read the three elements of an object definition. There must be
	 * a tab after the filename because the name can contain spaces.
	 */
   count = fscanf(*fp, "%s", obj_name);
   ...
   if (count != 3)
      sim_message(exit_program, "%s bad object definition (make sure a tab follows the filename).", obj_error_text);

   if (enter_group(obj_name, just_checking_element) != -1)
      sim_message(exit_program, "%s object name %s was already used as a group name.", obj_error_text, seg_name);

   if ((seg = enter_segment(sdm, seg_name)) == -2)
      sim_message(exit_program, "%s unknown segment name (%s) in object definition.", obj_error_text, seg_name);

   ph.body_segment = seg;
	
   if (read_polyhedron(&ph, clean_name) == code_bad)
      sim_message(exit_program, "Could not read %s...Exiting.", obj_name);
	 ...
	 */
   mstrcpy(&ph.name, obj_name);
   obj_num = enter_object(obj_name, defining_element);
