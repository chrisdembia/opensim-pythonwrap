/*******************************************************************************
   ...
   Copyright (c) 2000-2005 MusculoGraphics, a division of Motion Analysis Corp.
   All rights reserved.
...
#define STORE_EXTERNAL_FORCES_IN_GROUND_FRAME 0
#define COMBINE_SPRING_FORCES 1
#define STORE_SPRING_FORCES_IN_FLOOR_FRAME 1
...
void create_output_motion(char name[])
{
...
#if COMBINE_SPRING_FORCES
	dpBoolean *segmentHasSprings;
#endif
   ...
   /* time */
	...
	/* gencoords */
	if (params.outputOptions.gencoordValues == dpYes)
   {
		for (i = 0; i < sdm->nu; i++)
   ...
   /* muscle activation */
	if (params.outputOptions.muscleActivations == dpYes || params.outputOptions.optimizedMuscleActivations == dpYes)
   {
   ...
   /* muscle fiber length */
	if (params.outputOptions.muscleFiberLengths == dpYes)
   {
      for (i = 0; i < sdm->num_muscles; i++)
         if (sdm->muscles[i].output == dpYes)
            num_cols++;
   ...
   /* muscle fiber velocity */
	if (params.outputOptions.muscleFiberVelocities == dpYes)
   {
      for (i = 0; i < sdm->num_muscles; i++)
         if (sdm->muscles[i].output == dpYes)
            num_cols++;
   ...
   if (kinetics_data)
   {
#if STORE_EXTERNAL_FORCES_IN_GROUND_FRAME
   ...
   {
#if COMBINE_SPRING_FORCES
		/* All the spring forces on a body segment will be merged into one vector.
		 * The params.outputOptions.numSpringForces parameter is ignored.
		 */
		segmentHasSprings = (dpBoolean*)simm_calloc(sdm->num_body_segments, sizeof(dpBoolean));
      for (i = 0; i < sdm->num_springs; i++)
			segmentHasSprings[sdm->spring[i].segment + 1] = dpYes;
		for (i = 0; i < sdm->num_body_segments; i++)
		{
			if (segmentHasSprings[i] == dpYes)
				num_cols += 6;
		}
#else
	...
	if (params.outputOptions.muscleFiberLengths == dpYes)
   {
      for (i = 0; i < sdm->num_muscles; i++)
         if (sdm->muscles[i].output == dpYes)
         {
            sprintf(buffer, "%s_fiberlen", sdm->muscles[i].name);
            mstrcpy(&md.elementnames[count++], buffer);
         }
   }
	...
	if (params.outputOptions.muscleFiberVelocities == dpYes)
   {
      for (i = 0; i < sdm->num_muscles; i++)
         if (sdm->muscles[i].output == dpYes)
         {
            sprintf(buffer, "%s_fibervel", sdm->muscles[i].name);
            mstrcpy(&md.elementnames[count++], buffer);
         }
   }
   ...
   if (kinetics_data)
   {
      ...
      for (i = 0; i < kinetics_data->num_forces; i++)
      {
         sprintf(buffer, "%s_force_px%s", sdm->body_segment[kinetics_data->forces[i]->segment+1].name,
				get_function_suffix(&kinetics_data->forces[i]->appl_point[XX]));
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_force_py%s", sdm->body_segment[kinetics_data->forces[i]->segment+1].name,
				get_function_suffix(&kinetics_data->forces[i]->appl_point[YY]));
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_force_pz%s", sdm->body_segment[kinetics_data->forces[i]->segment+1].name,
				get_function_suffix(&kinetics_data->forces[i]->appl_point[ZZ]));
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_force_vx%s", sdm->body_segment[kinetics_data->forces[i]->segment+1].name,
				get_function_suffix(&kinetics_data->forces[i]->force_vec[XX]));
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_force_vy%s", sdm->body_segment[kinetics_data->forces[i]->segment+1].name,
				get_function_suffix(&kinetics_data->forces[i]->force_vec[YY]));
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_force_vz%s", sdm->body_segment[kinetics_data->forces[i]->segment+1].name,
				get_function_suffix(&kinetics_data->forces[i]->force_vec[ZZ]));
         mstrcpy(&md.elementnames[count++], buffer);
      }
#else
      /* Use this code to store the ground-based external
      ...
      for (i = 0; i < kinetics_data->num_forces; i++)
      {
         if (kinetics_data->forces[i]->segment == GROUND)
         {
            ...
            {
               sprintf(buffer, "%s_force_px%s", sdm->body_segment[sdm->force_matte[j].segment+1].name,
						get_function_suffix(&kinetics_data->forces[i]->appl_point[XX]));
               mstrcpy(&md.elementnames[count++], buffer);
               sprintf(buffer, "%s_force_py%s", sdm->body_segment[sdm->force_matte[j].segment+1].name,
						get_function_suffix(&kinetics_data->forces[i]->appl_point[YY]));
               mstrcpy(&md.elementnames[count++], buffer);
               sprintf(buffer, "%s_force_pz%s", sdm->body_segment[sdm->force_matte[j].segment+1].name,
						get_function_suffix(&kinetics_data->forces[i]->appl_point[ZZ]));
               mstrcpy(&md.elementnames[count++], buffer);
               sprintf(buffer, "%s_force_vx%s", sdm->body_segment[sdm->force_matte[j].segment+1].name,
						get_function_suffix(&kinetics_data->forces[i]->force_vec[XX]));
               mstrcpy(&md.elementnames[count++], buffer);
               sprintf(buffer, "%s_force_vy%s", sdm->body_segment[sdm->force_matte[j].segment+1].name,
						get_function_suffix(&kinetics_data->forces[i]->force_vec[YY]));
               mstrcpy(&md.elementnames[count++], buffer);
               sprintf(buffer, "%s_force_vz%s", sdm->body_segment[sdm->force_matte[j].segment+1].name,
						get_function_suffix(&kinetics_data->forces[i]->force_vec[ZZ]));
               mstrcpy(&md.elementnames[count++], buffer);
            }
         }
      ...
      {
         sprintf(buffer, "%s_torque_vx%s", sdm->body_segment[kinetics_data->torques[i]->segment+1].name,
				get_function_suffix(&kinetics_data->torques[i]->torque_vec[XX]));
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_torque_vy%s", sdm->body_segment[kinetics_data->torques[i]->segment+1].name,
				get_function_suffix(&kinetics_data->torques[i]->torque_vec[YY]));
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_torque_vz%s", sdm->body_segment[kinetics_data->torques[i]->segment+1].name,
				get_function_suffix(&kinetics_data->torques[i]->torque_vec[ZZ]));
         mstrcpy(&md.elementnames[count++], buffer);
      }
   }
   ...
   {
#if COMBINE_SPRING_FORCES
  #if STORE_SPRING_FORCES_IN_FLOOR_FRAME
		for (i = 0; i < sdm->num_body_segments; i++)
		{
			if (segmentHasSprings[i] == dpYes)
			{
				/* Find first spring which attaches to this segment and use its
				 * floor to write a segment name into the column label. This
				 * assumes that all springs attaching to one segment use either
				 * a single floor or multiple floors that are fixed to the same
				 * segment.
				 */
				int floorSegment = GROUND + 1;
				for (j = 0; j < sdm->num_springs; j++)
				{
					if (sdm->spring[j].segment + 1 == i)
					{
						floorSegment = sdm->spring_floor[sdm->spring[j].floor].segment + 1;
						break;
					}
				}
				sprintf(buffer, "%s_spring_force_px", sdm->body_segment[floorSegment].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_py", sdm->body_segment[floorSegment].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_pz", sdm->body_segment[floorSegment].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_vx", sdm->body_segment[floorSegment].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_vy", sdm->body_segment[floorSegment].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_vz", sdm->body_segment[floorSegment].name);
				mstrcpy(&md.elementnames[count++], buffer);
			}
		}
  #else
		for (i = 0; i < sdm->num_body_segments; i++)
		{
			if (segmentHasSprings[i] == dpYes)
			{
				sprintf(buffer, "%s_spring_force_px", sdm->body_segment[i].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_py", sdm->body_segment[i].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_pz", sdm->body_segment[i].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_vx", sdm->body_segment[i].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_vy", sdm->body_segment[i].name);
				mstrcpy(&md.elementnames[count++], buffer);
				sprintf(buffer, "%s_spring_force_vz", sdm->body_segment[i].name);
				mstrcpy(&md.elementnames[count++], buffer);
			}
		}
  #endif
		FREE_IFNOTNULL(segmentHasSprings);
#else
  #if STORE_SPRING_FORCES_IN_FLOOR_FRAME
      for (i = 0; i < MIN(params.outputOptions.numSpringForces, sdm->num_springs); i++)
      {
         sprintf(buffer, "%s_spring_force_px", sdm->body_segment[sdm->spring_floor[sdm->spring[i].floor].segment + 1].name);
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_spring_force_py", sdm->body_segment[sdm->spring_floor[sdm->spring[i].floor].segment + 1].name);
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_spring_force_pz", sdm->body_segment[sdm->spring_floor[sdm->spring[i].floor].segment + 1].name);
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_spring_force_vx", sdm->body_segment[sdm->spring_floor[sdm->spring[i].floor].segment + 1].name);
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_spring_force_vy", sdm->body_segment[sdm->spring_floor[sdm->spring[i].floor].segment + 1].name);
         mstrcpy(&md.elementnames[count++], buffer);
         sprintf(buffer, "%s_spring_force_vz", sdm->body_segment[sdm->spring_floor[sdm->spring[i].floor].segment + 1].name);
         mstrcpy(&md.elementnames[count++], buffer);
      }
  #else
      for (i = 0; i < MIN(params.outputOptions.numSpringForces, sdm->num_springs); i++)
	...
	if (params.outputOptions.jointTorques == dpYes)
   {
		for (i = 0; i < sdm->nq; i++)
		{
			if (sdm->q[i].output == dpYes)
         {
				if (params.outputOptions.traditionalInverse == dpNo)
					sprintf(buffer, "%s_torque_lin", sdm->q[i].name);
				else
					sprintf(buffer, "%s_torque_gcv", sdm->q[i].name);
            mstrcpy(&md.elementnames[count++], buffer);
         }
		}
   }
...
void store_motion_frame(double time, double state[])
{
	...
	if (params.outputOptions.muscleFiberLengths == dpYes)
   {
      for (i = 0; i < sdm->num_muscles; i++)
         if (sdm->muscles[i].output == dpYes)
            md.motiondata[col++][frame] = sdm->muscles[i].fiber_length;
   }
	...
	if (params.outputOptions.muscleFiberVelocities == dpYes)
   {
      for (i = 0; i < sdm->num_muscles; i++)
         if (sdm->muscles[i].output == dpYes)
            md.motiondata[col++][frame] = sdm->muscles[i].fiber_velocity;
   }
   ...
   {
#if COMBINE_SPRING_FORCES
		int *segmentSpringCount = (int*)simm_calloc(sdm->num_body_segments, sizeof(int));
		dpForceStruct *segmentSpringBin = (dpForceStruct*)simm_calloc(sdm->num_body_segments, sizeof(dpForceStruct));

		for (i = 0; i < sdm->num_springs; i++)
		{
			for (j = 0; j < 3; j++)
			{
				segmentSpringBin[sdm->spring[i].segment + 1].point[j] += sdm->spring[i].point[j];
				segmentSpringBin[sdm->spring[i].segment + 1].vector[j] += sdm->spring[i].force[j];
			}
			segmentSpringCount[sdm->spring[i].segment + 1]++;
		}

		for (i = 0; i < sdm->num_body_segments; i++)
		{
			if (segmentSpringCount[i] > 0)
			{
				int floorSegment = GROUND + 1;

				for (j = 0; j < 3; j++)
					segmentSpringBin[i].point[j] /= segmentSpringCount[i];

  #if STORE_SPRING_FORCES_IN_FLOOR_FRAME
				/* Find first spring which attaches to this segment and use its
				 * floor segment to output the combined spring force in. This
				 * assumes that all springs attaching to one segment use either
				 * a single floor or multiple floors that are fixed to the same
				 * segment.
				 */
				...
				sdvcopy(segmentSpringBin[i].point, pt);
				convert_point(pt, i - 1, floorSegment - 1);
				sdvadd(sdm->body_segment[floorSegment].mass_center, pt, pt);
				sdtrans(i - 1, segmentSpringBin[i].vector, floorSegment - 1, frc);
  #else
				sdvadd(sdm->body_segment[i].mass_center, segmentSpringBin[i].point, pt);
				sdvcopy(segmentSpringBin[i].vector, frc);
  #endif
				md.motiondata[col++][frame] = pt[XX];
				md.motiondata[col++][frame] = pt[YY];
				md.motiondata[col++][frame] = pt[ZZ];
		...
		FREE_IFNOTNULL(segmentSpringCount);
		FREE_IFNOTNULL(segmentSpringBin);
#else
  #if STORE_SPRING_FORCES_IN_FLOOR_FRAME
      for (i = 0; i < MIN(params.outputOptions.numSpringForces, sdm->num_springs); i++)
      {
			sdvcopy(sdm->spring[i].point, pt);
			convert_point(pt, sdm->spring[i].segment, sdm->spring_floor[sdm->spring[i].floor].segment);
         sdvadd(sdm->body_segment[sdm->spring_floor[sdm->spring[i].floor].segment + 1].mass_center, pt, pt);
			sdtrans(sdm->spring[i].segment, sdm->spring[i].force, sdm->spring_floor[sdm->spring[i].floor].segment, frc);
			md.motiondata[col++][frame] = pt[XX];
			md.motiondata[col++][frame] = pt[YY];
			md.motiondata[col++][frame] = pt[ZZ];
			md.motiondata[col++][frame] = frc[XX];
      ...
      for (i = 0; i < MIN(params.outputOptions.numSpringForces, sdm->num_springs); i++)
      {
         ...
         sdvadd(sdm->body_segment[sdm->spring[i].segment + 1].mass_center, sdm->spring[i].point, pt);
         md.motiondata[col++][frame] = pt[XX];
         md.motiondata[col++][frame] = pt[YY];
         md.motiondata[col++][frame] = pt[ZZ];
   ...
   {
      for (i = 0; i < md.num_elements; i++)
         fprintf(md.fp, "%21.16lf\t", md.motiondata[i][frame]);
