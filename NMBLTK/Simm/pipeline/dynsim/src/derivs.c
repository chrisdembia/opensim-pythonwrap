/*******************************************************************************
   ...
   Routines:
      ...
      muscle_deriv_func10 : 1 state (fiber_len), used only for optimize_muscle_activations()
		                 ...
		                 double dstate[], double* muscle_force)
{
   ...
   double c_tau, mass_param, c_act_1, c_act_2;
	...
	static int c_tau_index = 0, mass_param_index = 0, c_act_1_index = 0, c_act_2_index = 0;

	if (first_time)
	{
		c_tau_index = get_muscle_param_index(ms,"timescale");
		mass_param_index = get_muscle_param_index(ms,"mass");
	...
	mass_param = *(ms->dynamic_params[mass_param_index]);
		                 ...
		                 double dstate[], double* muscle_force)
{
   ...
   double c_tau, c_act_1, c_act_2, c_heat_r, c_heat_c1, c_heat_alpha;
	static int first_time = 1;
	static int c_tau_index = 0, c_act_1_index = 0, c_act_2_index = 0;
	static int c_heat_r_index = 0, c_heat_c1_index = 0, c_heat_alpha_index = 0;
	...
	if (first_time)
	{
		...
		c_act_1_index = get_muscle_param_index(ms,"activation1");
		c_act_2_index = get_muscle_param_index(ms,"activation2");
		c_heat_r_index = get_muscle_param_index(ms,"heat_r");
		c_heat_c1_index = get_muscle_param_index(ms,"heat_c1");
		c_heat_alpha_index = get_muscle_param_index(ms,"heat_alpha");
		first_time = 0;
	}
	...
	c_act_1 = *(ms->dynamic_params[c_act_1_index]);
	c_act_2 = *(ms->dynamic_params[c_act_2_index]);
	c_heat_r = *(ms->dynamic_params[c_heat_r_index]);
	c_heat_c1 = *(ms->dynamic_params[c_heat_c1_index]);
	c_heat_alpha = *(ms->dynamic_params[c_heat_alpha_index]);
...
/* Model 10 is specifically designed to work with optimize_muscle_activations().
 * It is similar to model 4, except activation is not included because that
 * is the variable being optimized. Also, it is important to set
 * ms->force to 0.0 at the end of this function so that no force is applied
 * to the body segments. This muscle model is for use during inverse dynamics,
 * and applying muscles forces would change the joint torques that are
 * calculated and used in the optimization. Thus this muscle model is used
 * only to calculate up-to-date values of muscle_tendon_length, fiber_length,
 * and fiber_velocity. The actual force in the muscle is calculated by a
 * different function, calc_optimized_muscle_force(), using the state
 * variables calculated by this function.
 */
                        ...
                        double dstate[], double* muscle_force)
{
   int numStates = 1;
   int fiber_length = 0;
   double tendon_force, active_force;
   double passive_force, velocity_dependent_force;
   double norm_fiber_length, norm_fiber_length_deriv, norm_tendon_length, ca;
   double norm_muscle_tendon_length, pennation_angle;
   double h, w, new_fiber_length, new_pennation_angle, new_ca;
   double c_tau;
	static int first_time = 1;
	static int c_tau_index = 0;
   ...
   /* Normalize the muscle state derivative */
   norm_fiber_length = state[fiber_length] / (*(ms->optimal_fiber_length));
  
   pennation_angle = calc_pennation(norm_fiber_length, 1.0, (*(ms->pennation_angle)));
   ca = cos(DEG_TO_RAD*pennation_angle);
   norm_muscle_tendon_length = calc_muscle_tendon_length(ms) / (*(ms->optimal_fiber_length));
   norm_tendon_length = norm_muscle_tendon_length - norm_fiber_length * ca;
   tendon_force = calc_tendon_force(ms, norm_tendon_length);
   passive_force = calc_nonzero_passive_force(ms, norm_fiber_length, 0.0);
   ...
   if (EQUAL_WITHIN_ERROR(ca,0.0))
   {
      ...
      else
      {
         ...
         w = (*(ms->optimal_fiber_length)) * sin((*(ms->pennation_angle)) * DEG_TO_RAD);
         new_fiber_length = sqrt(h*h + w*w) / (*(ms->optimal_fiber_length));
         new_pennation_angle = calc_pennation(new_fiber_length, 1.0, (*(ms->pennation_angle)));
         new_ca = cos(new_pennation_angle);
         norm_fiber_length_deriv = calc_muscle_tendon_velocity(ms) *
                                      c_tau / (*(ms->optimal_fiber_length)) * new_ca;
         ms->fiber_velocity = norm_fiber_length_deriv;
      }
   }
   else
   {
      velocity_dependent_force = tendon_force / ca - passive_force;
      norm_fiber_length_deriv = calc_fiber_velocity(ms, ms->dynamic_activation,
				                                        active_force, velocity_dependent_force);
   }

   /* Un-normalize the muscle state derivative */
   dstate[fiber_length] = norm_fiber_length_deriv * (*(ms->optimal_fiber_length)) / c_tau;

   *muscle_force = tendon_force * (*(ms->max_isometric_force));

   /* Set muscle force to zero so that it won't apply any force to the segments. */
   ms->force = 0.0;
