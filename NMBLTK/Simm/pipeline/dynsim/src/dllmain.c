/*******************************************************************************
   ...
   Copyright (c) 2004-5 MusculoGraphics, a division of Motion Analysis Corp.
   All rights reserved.
...
DLL int dpSetModel(dpModelStruct* model)
{
   ...
   /* If muscle activations are being optimized, force the simulation
    * to use muscle model 10 (index 9 in the zero-based array), which is
    * specifically designed to work with optimize_muscle_activations().
    */
   if (params.outputOptions.optimizedMuscleActivations == dpYes)
   {
      if (sdm->default_muscle.muscle_model_index)
         *(sdm->default_muscle.muscle_model_index) = 9;

      for (i = 0; i < sdm->num_muscles; i++)
      {
         if (sdm->muscles[i].muscle_model_index &&
             sdm->muscles[i].muscle_model_index != sdm->default_muscle.muscle_model_index)
         {
            *(sdm->muscles[i].muscle_model_index) = 9;
         }
      }
   }

	/* Count the number of muscle states, and quit the simulation
	 * if there are fewer than zero (meaning that there was an
	 * error counting the states).
	 */
   num_muscle_states = count_muscle_states(sdm);
	if (num_muscle_states < 0)
		return -1;
   ...
   if (dataCallbackFunc)
   {
	...
	{
		FILE* fp;
		dpBoolean newInverse = sdm->newInverseSimulation; // save a copy of this flag

		/* For an inverse simulation, activate prescribed motion and prescribe
		 * the Qs, Us, and As according to the input motion. Then write the
		 * initial Qs and Us to a file. This is useful if, for example, you
	    * later want to use the joint torques to drive a forward simulation
		...
		fp = fopen("initialConditions.txt","w");
		for (i = 0; i < sdm->nq; i++)
			fprintf(fp, "%s %21.16lf %21.16lf\n", sdm->q[i].name, y[i] * sdm->q[i].conversion, dy[i] * sdm->q[i].conversion);
		fclose(fp);

		/* Set the newInverseSimulation flag back to what it was. */
		sdm->newInverseSimulation = newInverse;
		if (newInverse == dpYes)
   ...
   if (params.outputOptions.jointReactionForces == dpYes || params.outputOptions.jointReactionTorques == dpYes)
	...
	if (params.outputOptions.jointTorques == dpYes || params.outputOptions.optimizedMuscleActivations == dpYes)
	{
		if (sdm->newInverseSimulation == dpYes)
			calc_initial_joint_torques(t, y, dy, kinetics_data);
		else
   ...
   if (params.outputOptions.muscleMomentArms == dpYes || params.outputOptions.muscleJointTorques == dpYes ||
       params.outputOptions.totalMuscleJointTorques == dpYes || params.outputOptions.optimizedMuscleActivations == dpYes)
      calc_muscle_moment_arms(t, y);

   if (params.outputOptions.optimizedMuscleActivations == dpYes)
      optimize_muscle_activations(t, y);
...
DLL int dpRunSimulation(int code)
{
	...
	for (t = params.startTime; t < params.endTime - TINY_TIME; )
   {
      ...
      if (params.outputOptions.jointTorques == dpYes || params.outputOptions.optimizedMuscleActivations == dpYes)
		{
			if (sdm->newInverseSimulation == dpYes)
			{
				if (t < params.endTime - TINY_TIME)
				{
				calc_next_joint_torques(t, y, dy, kinetics_data);
				calc_corrective_torques(t, y, dy, sdm, kinetics_data);
				}
				else
				{
					/* You're at the end of the simulation, so you don't need to
					 * calculate corrective torques for the next time step. Just get
					 * the corrected torques from this time step and store them in
					 * the sdm->q[] array so they can be written to the output motion
					 * file and used by the static muscle optimization function.
					 */
					for (i = 0; i < sdm->nq; i++)
					{
						if (kinetics_data->q_torques[i])
							sdm->q[i].torque = kinetics_data->q_torques[i]->y[kinetics_data->q_torques[i]->numpoints-1];
					}
				}
			}
			else
      ...
      /* Moment arms should be calculated near the end because the function
       * calls sdstate() which resets all of the accelerations to zero.
       */
      if (params.outputOptions.muscleMomentArms == dpYes || params.outputOptions.muscleJointTorques == dpYes ||
          params.outputOptions.totalMuscleJointTorques == dpYes || params.outputOptions.optimizedMuscleActivations == dpYes)
...
int sim_message(ErrorAction action, const char* format, ...)
{
   ...
   strncat(mess.text, buffer3, sizeof(mess.text) - 6);
