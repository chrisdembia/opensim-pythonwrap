...
int sdumotion(double t, double q[], double u[])
{
   ...
   set_fixed_gencoords(sdm);
	...
	if (sdm->newInverseSimulation == dpNo)
...
void set_up_kinetics_input(char filename[], MotionData** data)
{
   ...
   if (i == sdm->nq)
   {
       ...
       * interval) for the integrator. If the startTime and endTime are already
		 * equal to the times of the first and last frames, don't change stepSize
		 * because the user may have changed it to re-sample the data.
       */
		if (NOT_EQUAL_WITHIN_ERROR(params.startTime, (*data)->q_data[index_pres]->x[0]) ||
			 NOT_EQUAL_WITHIN_ERROR(params.endTime, (*data)->q_data[index_pres]->x[(*data)->q_data[index_pres]->numpoints - 1]))
		{
      ...
      sim_message(none, "      step size to  %lf", params.stepSize);
		...
		/* If the "new" style of joint torque calculation was selected,
		 * then set the newInverseSimulation flag and make room for the torque curves.
		 */
		if (params.outputOptions.traditionalInverse == dpNo)
		{
			sdm->newInverseSimulation = dpYes;
		add_torques_to_motion_data(sdm, *data);
   }
   }
	else
	{
